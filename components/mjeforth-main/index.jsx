...
var main = React.createClass({
  ...
  onClick: function (){
    var result=vm.exec(jefTst.value);
    console.log(result);
    if(jefTst)
      jefTst.value='';
  },
  prevInp: function (){
    if(vm.iCmd>0)
      jefTst.value=vm.cmds[--vm.iCmd];
  },
  nextInp: function (){
    if(vm.iCmd<vm.cmds.length-1)
      jefTst.value=vm.cmds[++vm.iCmd];
  },
  clearJefTst: function (){
    jefTst.value='';
  },
  render: function() {
    return (
...
<button type="button" onClick={this.onClick       }>run jeforth</button>
<button type="button" onClick={this.prevInp       }>&lt;</button>
<button type="button" onClick={this.nextInp       }>&gt;</button>
<button type="button" onClick={this.clearJefTst   }>clear</button>
<button type="button" onClick={this.saveJefTst    }>save</button><br/><br/>
